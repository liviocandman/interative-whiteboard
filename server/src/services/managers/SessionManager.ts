// server/src/services/managers/SessionManager.ts
...
export class SessionManager {
  ...
  // Retorna todas as sessões de uma sala
  ...
  private async saveSessionToRedis(session: Session): Promise<void> {
    ...
    // Salva room mapping separadamente para queries rápidas
    if (session.roomId) {
      await this.redis.setWithTTL(
  ...
  // Método para recuperar sessão do Redis (útil para restart do servidor)
